...
function abToBase64(buf: ArrayBuffer) {
  ...
  // @ts-ignore Edge Runtime では btoa が使える
  return btoa(s);
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2) OpenAI (画像マルチ) — 型は any にキャストしてビルドエラー回避
    const systemPrompt = [
      'あなたは中古リユース査定AI「カンテノ」。',
    ...
    const mercariTitleRaw = [parsed.brand ?? '', parsed.title_guess ?? '', parsed.material ?? '', parsed.period ?? '']
      .filter(Boolean)
      .join(' ');
