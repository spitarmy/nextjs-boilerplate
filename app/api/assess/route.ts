...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!urls.length) {
      return NextResponse.json({ ok:false, error:'image_urls（配列）または image_url が必要です。' }, { status:400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ ok:false, error:'openai_error', detail, debug:{ parts: imageParts.length } }, { status:500 });
    }
